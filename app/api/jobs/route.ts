...
import type { EmployerDoc, EmployerPackageDoc, EmploymentType, JobDoc } from '@/server/db/models';
...
async function nextJobUniqueId() {
  ...
  const latest = await jobs
    ...
    .limit(1)
    .next();
  const maxNumber = latest ? Number(latest.jobUniqueId.replace('YEC-', '')) || 0 : 0;
...
export async function GET(req: NextRequest) {
  ...
  const rows = await jobs.find({ status: 'active' }).sort({ postedAt: -1 }).toArray();
...
export async function POST(req: NextRequest) {
  ...
  if (!String(title ?? '').trim()) return NextResponse.json({ error: 'Job title is required.' }, { status: 400 });
  if (!String(company ?? '').trim()) return NextResponse.json({ error: 'Company name is required.' }, { status: 400 });
  if (!String(city ?? '').trim()) return NextResponse.json({ error: 'City is required.' }, { status: 400 });
  if (!String(province ?? '').trim()) return NextResponse.json({ error: 'Province is required.' }, { status: 400 });
  if (!String(employmentType ?? '').trim()) return NextResponse.json({ error: 'Employment type is required.' }, { status: 400 });
  if (!String(category ?? '').trim()) return NextResponse.json({ error: 'Category is required.' }, { status: 400 });
  if (!String(jobPostingDate ?? '').trim() || Number.isNaN(Date.parse(String(jobPostingDate).trim()))) {
    return NextResponse.json({ error: 'A valid job posting date is required.' }, { status: 400 });
  }
  if (!Number.isInteger(durationDays) || durationDays <= 0) {
    return NextResponse.json({ error: 'Ad duration is required.' }, { status: 400 });
  }
  if (!String(descriptionHtml ?? '').trim()) {
    return NextResponse.json({ error: 'Job description is required.' }, { status: 400 });
  }
  if (!byEmail && !byMail && !byPhone && !inPerson) {
    return NextResponse.json({ error: 'Select at least one application method.' }, { status: 400 });
  }
  if (byEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue)) {
    return NextResponse.json({ error: 'A valid application email is required.' }, { status: 400 });
  }
  if (byMail && !mailingValue) return NextResponse.json({ error: 'A mailing address is required when By Mail is selected.' }, { status: 400 });
  if (byPhone && !phoneValue) return NextResponse.json({ error: 'A phone number is required when By Phone is selected.' }, { status: 400 });
  if (inPerson && (!inPersonAddressValue || !inPersonFromValue || !inPersonToValue)) {
    return NextResponse.json({ error: 'In Person address and time range are required when In Person is selected.' }, { status: 400 });
  }

  const employers = await collection<EmployerDoc>('employers');
  const employer = await employers.findOne({ userId: user.id });
  if (!employer) return NextResponse.json({ error: 'Employer profile not found. Please login again.' }, { status: 404 });
  ...
  if (!employerPackage) return NextResponse.json({ error: 'No active job posting package found.' }, { status: 400 });
  if (employerPackage.status !== 'Active') return NextResponse.json({ error: 'Your package is not active.' }, { status: 403 });
  // if (employerPackage.jobCredits <= 0) {
  //   return NextResponse.json({ error: 'You have 0 job credits remaining. Please purchase more credits.' }, { status: 403 });
  // }
  ...
  await jobs.insertOne({
    ...
    jobPostingDate: String(jobPostingDate ?? '').trim() || null,
