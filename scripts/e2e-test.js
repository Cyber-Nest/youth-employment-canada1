(async () => {
  ...
  console.log('[e2e] Calling /api/auth/me again to verify credits');
  const meRes2 = await fetch(`${base}/api/auth/me`, { method: 'GET', headers: { cookie } });
  console.log('[e2e] /api/auth/me body:', await meRes2.text());

  console.log('[e2e] Submitting a second job (should fail with 403 due to 0 credits)');
  const jobRes2 = await fetch(`${base}/api/jobs`, { method: 'POST', headers: { 'content-type': 'application/json', cookie }, body: JSON.stringify(jobBody) });
  console.log('[e2e] Second /api/jobs status:', jobRes2.status);
  console.log('[e2e] Second /api/jobs body:', await jobRes2.text());
})();
